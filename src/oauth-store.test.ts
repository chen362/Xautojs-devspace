...
try {
  ...
  await testOidcModeDoesNotCreateLocalStore(join(root, "oidc"));
} finally {
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  const firstProvider = new SingleUserOAuthProvider({ ...oauthConfig, stateDir }, mcpUrl);
  ...
  const secondProvider = new SingleUserOAuthProvider({ ...oauthConfig, stateDir }, mcpUrl);
  try {
...
async function testOidcModeDoesNotCreateLocalStore(stateDir: string): Promise<void> {
